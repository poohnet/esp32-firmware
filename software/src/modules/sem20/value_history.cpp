...
void ValueHistory::setup()
{
    ...
    ws.addOnConnectCallback([this](WebSocketsClient client) {
        ...
        buf_written += snprintf(buf + buf_written, buf_size - buf_written, "%s", "{\"topic\":\"sem20/live\",\"payload\":");
        ...
        buf_written += snprintf(buf + buf_written, buf_size - buf_written, "%s", "{\"topic\":\"sem20/history\",\"payload\":");
...
void ValueHistory::add_sample(float sample)
{
    ...
    {
        char *buf;
        int buf_written = asprintf(&buf, "{\"topic\":\"sem20/live_samples\",\"payload\":{\"samples_per_second\":%f,\"samples\":[%d]}}\n", samples_per_second(), (int)val);
    ...
    if (live.used() == 1) {
        task_scheduler.scheduleWithFixedDelay([this, val_min](){
            ...
            char *buf;
            int buf_written;
            const char *prefix = "{\"topic\":\"sem20/history_samples\",\"payload\":{\"samples\":";

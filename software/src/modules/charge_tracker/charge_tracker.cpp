...
static char *tracked_charge_to_string(char *buf, ChargeStart cs, ChargeEnd ce, bool english, display_name_entry *display_name_cache)
{
    ...
    if (cs.electricity_price == 0) {
        ...
        buf += 1 + sprintf_u(buf, "%d%c%02d", cs.electricity_price / 100, english ? '.' : ',', cs.electricity_price % 100);
    }
...
search_done:
        ...
        written = sprintf_u(stats_head, "%s: %d.%02d€ %s",
        ...
        init_pdf_generator(&request,
            ...
            while (any_charges_tracked && current_file <= last_file) {
                ...
                for (; current_charge < (CHARGE_RECORD_MAX_FILE_SIZE / CHARGE_RECORD_SIZE); ++current_charge) {
                    ...
                    table_lines_head = tracked_charge_to_string(table_lines_head, cs, ce, english, display_name_cache);
                    ++lines_generated;
                }

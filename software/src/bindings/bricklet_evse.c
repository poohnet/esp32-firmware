...
int tf_evse_get_response_expected(TF_EVSE *evse, uint8_t function_id, bool *ret_response_expected) {
    ...
    switch (function_id) {
        ...
        case TF_EVSE_FUNCTION_SET_BOOST_CURRENT:
            ...
            break;
        case TF_EVSE_FUNCTION_SET_PWM_OVERRIDE:
            if (ret_response_expected != NULL) {
        ...
        case TF_EVSE_FUNCTION_WRITE_UID:
            if (ret_response_expected != NULL) {
                *ret_response_expected = (evse->response_expected[1] & (1 << 6)) != 0;
            }
            break;
        default:
...
int tf_evse_set_pwm_override(TF_EVSE *evse, uint16_t pwm_override) {
    ...
    bool _response_expected = true;
    tf_evse_get_response_expected(evse, TF_EVSE_FUNCTION_SET_PWM_OVERRIDE, &_response_expected);
    tf_tfp_prepare_send(evse->tfp, TF_EVSE_FUNCTION_SET_PWM_OVERRIDE, 2, _response_expected);

    uint8_t *_send_buf = tf_tfp_get_send_payload_buffer(evse->tfp);

    pwm_override = tf_leconvert_uint16_to(pwm_override); memcpy(_send_buf + 0, &pwm_override, 2);
...
int tf_evse_get_pwm_override(TF_EVSE *evse, uint16_t *ret_pwm_override) {
    ...
    bool _response_expected = true;
    tf_tfp_prepare_send(evse->tfp, TF_EVSE_FUNCTION_GET_PWM_OVERRIDE, 0, _response_expected);
    ...
    if (_result & TF_TICK_PACKET_RECEIVED) {
        ...
        if (_error_code != 0 || _length != 2) {
            ...
            if (ret_pwm_override != NULL) { *ret_pwm_override = tf_packet_buffer_read_uint16_t(_recv_buf); } else { tf_packet_buffer_remove(_recv_buf, 2); }
        }
        tf_tfp_packet_processed(evse->tfp);
    }
